...
% We use a boolean variable to check whether the vehicle is in the merge point or not. 
...
\begin{itemize}[]
    ...
    \item[2)] The protocol is in prioritized mode and there is a vehicle in the merge point, when the top through lane vehicle whose status is approaching stops before the merge point. %2
    \item[3)] The protocol is in fair mode and the current turn is the non-through lane's, when the top through lane vehicle whose status is approaching stops before the merge point. %3
    \item[4)] The protocol is in fair mode, the current turn is the through lane's and there is a vehicle in the merge point, when the top through lane vehicle whose status is approaching stops before the merge point. %3
    \item[5)] The protocol is in prioritized mode and there is no vehicle in the merge point, when the top through lane vehicle whose status is either approaching or stopped starts crossing the merge point. %4
    \item[6)] The protocol is in fair mode, the current turn is the through lane's and there is no vehicle in the merge point, when the top through lane vehicle whose status is either approaching or stopped starts crossing the merge point. %5
    \item[7)] The protocol is in fair mode, the current turn is the non-through lane's, there is no vehicle in the merge point and there is no non-through lane vehicle whose status is either approaching or stopped, when the top through lane vehicle whose status is stopped starts crossing the merge point. %6
    \item[8)] The vehicle completes the crossing. %7
\end{itemize}
